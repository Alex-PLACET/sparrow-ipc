...
{
    template <sparrow::zoned_time_without_timezone_type T>
    [[nodiscard]] sparrow::timestamp_without_timezone_array<T> deserialize_timestamp_without_timezone(
    ...
    [[nodiscard]] ArrayType<T> deserialize_timestamp_array(
        const org::apache::arrow::flatbuf::RecordBatch& record_batch,
        std::span<const uint8_t> body,
        std::string_view name,
        const std::optional<std::vector<sparrow::metadata_pair>>& metadata,
        bool nullable,
        size_t& buffer_index,
        const ::flatbuffers::String& timezone
    )
    {
        std::string format{data_type_to_format(
